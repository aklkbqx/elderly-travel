...
<body>
    ...
    <div class='container mb-5' style='margin-top:10rem'>
        <div class='d-flex'>
            <a href="javascript:window.history.back()" class='text-decoration-none text-dark d-flex align-items-center gap-1'>
        ...
        <?php if(isset($_GET["news_id"])){
            ...
            ?>
            <div class='bg-light p-2 rounded-xl shadow border mt-2'>
                ...
                <div class='p-2'>
                    ...
                    <div class='d-flex justify-content-between mt-4 mb-2'>
                        <a href="./api/news.php?like&news_id=<?= $news_id ?>" class='text-decoration-none d-flex align-items-center gap-1 btn btn-outline-light bg-white border'>
                            <img src="<?= imagePath("web_images/icons",$heart) ?>" width='30px' height='30px' class='object-fit-cover'>
                            <div class='text-dark'><?= sql("SELECT COUNT(*) as count FROM news_likes WHERE news_id = ?",[$news_id])->fetch()["count"] ?></div>
                        </a>
                        <div class='d-flex align-items-center gap-1'>
                            <img src="<?= imagePath("web_images/icons","eye.png") ?>" width='30px' height='30px' class='object-fit-cover'>
                            <div>จำนวนผู้เข้าชม <span><?= $news["visitors"] ?></span></div> 
                        </div>
                    </div>
                    
                    <form method='post' action='api/news.php?comment&news_id=<?= $news_id ?>' class='d-flex align-items-center gap-2 border-bottom pb-4'>
